...
def filter_df(
    ...
    operator: str | None="All",
    time_of_day: str | None="All",
    aircraft: str | None="All"
) -> pd.DataFrame:
    """
    ...
    Parameters:
        ...
        operator (str): Operator filter ("All" for no filter).
        time_of_day (str): Time of day filter ("All" for no filter).
        aircraft (str): Aircraft filter ("All" for no filter).
...
def count_by(
    ...
    Parameters:
        ...
        label (str): Name to assign to the grouped column.
        n (int, optional): Return only the top N rows.
        ascending (bool): Sort order for counts.

    Returns:
        pd.DataFrame: Counts per group with optional top N filtering.
    """
    out = (
...
def get_home_counts(df_states: pd.DataFrame) -> dict[str, pd.DataFrame]:
    return {
...
def get_heatmap_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Parameters:
        df (pd.DataFrame): Pre-filtered DataFrame.

    Returns:
        pd.DataFrame: DataFrame with valid latitude and longitude columns.
    """
...
def prepare_scatter_data(df: pd.DataFrame, x_col: str) -> pd.DataFrame:
    ...
    Parameters:
        df (pd.DataFrame): Pre-filtered DataFrame.
        x_col (str): Column to use as X variable.

    Returns:
        pd.DataFrame: Cleaned DataFrame with log-transformed variables.
    """
